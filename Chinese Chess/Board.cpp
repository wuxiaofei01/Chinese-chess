...
Board::Board(QWidget *parent) : QWidget(parent)//构造函数，初始棋子
{
    this->_r = 20;
...
void Board::init(bool bRedSide)//对棋子赋值活，颜色赋值
{
    for(int i=0; i<32; ++i)
    {
        _s[i].init(i);
...
void Board::drawStone(QPainter &p)//画棋子
{
    for(int i=0; i<32; i++)
    {
        drawStone(p, i);
...
void Board::drawPlace(QPainter &p)//九宫格
{
...
bool Board::getClickRowCol(QPoint pt, int &row, int &col)//得到点击的位置
{
    for(row=0; row<=9; ++row)
    {
        for(col=0; col<=8; ++col)
...
void Board::saveStep(int moveid, int killid, int row, int col, QVector<Step*>& steps)//存放走棋信息
{
    GetRowCol(row1, col1, moveid);
    Step* step = new Step;
    step->_colFrom = col1;
...
void Board::click(QPoint pt)//得到点击的点
{
    int row, col;
...
int Board::getStoneId(int row, int col)//返回棋子的代号
{
    for(int i=0; i<32; ++i)
    {
